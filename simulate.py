...
class Modal:
    def __init__ (self):
        ...
        self.far_search = [(2,0),[-2,0],(0,2),(0,-2)]

        self.turn = 1#回合数目

        self.people_rate = 0.2    #产生人的概率
        ...
        for x in range(self.mapsize[0]):
            for y in range(self.mapsize[1]):
                if x==start[0] and y==start[1]:         #初始化赋值
                    ...
                    continue
                if self.possibility(self.people_rate):  #判断是为人
                    self.map_temp[x][y] = 1
    ...
    def infect(self,x,y):
        ...
        for id in self.search:  #对周围8个方向进行遍历
            ...
            if self.map_temp[new_x][new_y] == 4 and self.possibility(self.people_getsick_cure): #恢复后的患病
                self.map_temp[new_x][new_y] = 2  #感染
                self.rec_sick += 1
                self.rec_infect_cnt += 1
    ...
    def cure(self,x,y):
        if self.possibility(self.cure_rate):#治愈
            self.map_temp[x][y] = 4   #变成恢复者
            self.rec_sick -= 1

    def work_people(self,x,y):#012 空地 平民 患者   #进行人员流动
